...
require_relative "system/errbug/container"
...
namespace :db do
  ...
  DB = Sequel.connect(Errbug::Container.settings.database_url)
  ...
  namespace :structure do
    ...
    task :dump do
      ...
      dump = `pg_dump -U #{uri.user} -h #{uri.hostname} -s -x -O #{uri.path.tr("/", "")}`
